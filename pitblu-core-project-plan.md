...
Build `pitblu-core`, a headless, API-first service running on a Raspberry Pi that discovers, connects to and monitors a Weber iGrill over Bluetooth Low Energy. It exposes all administrative and operational functionality through a REST API and publishes reusable telemetry through MQTT.

The later Pitblu web application is a separate project. It will consume the API and MQTT data and provide cook sessions, history, graphs, alarms and other user-facing features.
...
The default base topic is `pitblu`, but it is configurable. MQTT schema version `v1` is independent of the REST API version.
...
The retained service-availability topic uses MQTT Last Will and Testament. No administrative commands are accepted over MQTT. A consumer can subscribe to all temperature readings using `pitblu/v1/devices/+/probes/+/temperature`.
...
- Application and virtual environment under `/opt/pitblu-core/`
- Non-secret configuration under `/etc/pitblu-core/`
- Protected secret environment file
- Administrative state under `/var/lib/pitblu-core/`
...
Mosquitto is installed locally on `pitblu` and has been tested successfully.
...
- `pitblu-core` MQTT identity
- ACL limited to `pitblu/#`
...
- Repository: `moodywaters/pitblu`, with `pitblu-core/` and future `pitblu-web/` components
