# pitblue-app API and integration guide

`pitblue-app` is Pitblu's API-first cook service. The official operator, display
...
values of at least 32 characters through `PITBLUE_APP_OPERATOR_TOKEN`,
`PITBLUE_APP_DISPLAY_TOKEN` and, when needed,
`PITBLUE_APP_INTEGRATION_TOKEN`.
...
External clients integrate with `pitblue-app`, not `pitblu-core`, for Cook-aware
