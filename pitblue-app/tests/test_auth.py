...
def test_public_surfaces_do_not_issue_credentials(tmp_path):
    ...
    with client:
        ...
        asset_name = next(
            path.name
            for path in (
                Path(__file__).parents[1] / "src" / "pitblue_app" / "static" / "assets"
            ).glob("index-*.js")
        )
...
def test_environment_tokens_are_required_distinct_and_long(monkeypatch):
    monkeypatch.delenv("PITBLUE_APP_OPERATOR_TOKEN", raising=False)
    monkeypatch.delenv("PITBLUE_APP_DISPLAY_TOKEN", raising=False)
    try:
    ...
    monkeypatch.setenv("PITBLUE_APP_OPERATOR_TOKEN", OPERATOR)
    monkeypatch.setenv("PITBLUE_APP_DISPLAY_TOKEN", OPERATOR)
    try:
        AccessControl.from_environment()
    except RuntimeError as exc:
