...
def test_optional_systemd_example_is_unprivileged_and_hardened():
    ...
    unit = (root / "deploy" / "pitblue-app.service").read_text()
    assert "User=pitblue-app" in unit
    assert "NoNewPrivileges=true" in unit
    assert "ProtectSystem=strict" in unit
    assert "ReadWritePaths=/var/lib/pitblue-app" in unit
...
def test_environment_example_contains_no_secret():
    ...
    assert "PITBLUE_APP_DATABASE=/var/lib/pitblue-app/state.sqlite3" in environment
...
def test_frontend_is_built_static_and_production_has_no_node_runtime():
    ...
    assert (root / "src" / "pitblue_app" / "static" / "index.html").is_file()
    assert any((root / "src" / "pitblue_app" / "static" / "assets").glob("index-*.js"))
