...
from pitblue_app import __version__
from pitblue_app.auth import AccessControl, Scope
from pitblue_app.core_client import PitbluCoreClient, ThermometerGateway
from pitblue_app.events import ApplicationEvents
from pitblue_app.models import (
    AssignmentCreate,
    CookCreate,
    CookEventCreate,
    CookPatch,
...
from pitblue_app.service import CookService
from pitblue_app.store import Store, public_row
...
def create_app(
    *,
    ...
    database = store or Store(os.getenv("PITBLUE_APP_DATABASE", "pitblue-app.sqlite3"))
    ...
    async def response_safety(request: Request, call_next):  # type: ignore[no-untyped-def]
        ...
        public = (
            ...
            or request.url.path in {"/index.html", "/manifest.webmanifest", "/icon.svg", "/sw.js"}
    ...
    async def pwa_asset(asset_name: str) -> FileResponse:
        if not re.fullmatch(
            r"(?:index\.html|manifest\.webmanifest|icon\.svg|sw\.js|workbox-[a-zA-Z0-9_-]+\.js)",
            asset_name,
        ):
...
def run() -> None:
    ...
    uvicorn.run(
        ...
        host=os.getenv("PITBLUE_APP_BIND", "0.0.0.0"),
        port=int(os.getenv("PITBLUE_APP_PORT", "8081")),
    )
