...
function HardwareReadiness({system}: {system: SystemState}) {
  const probeCount = system.core.devices.reduce((total, device) => total + device.probes.filter(probe => probe.available && probe.fresh !== false).length, 0);
...
function Setup({cook, system, disabled, api, mutate}: {cook: Cook; system: SystemState; disabled: boolean; api: PitbluApi; mutate: <T>(path: string, body?: unknown, method?: string) => Promise<T>}) {
  ...
  return <div className="setup-grid"><div className="wide"><HardwareReadiness system={system}/></div>
    ...
    <section className="card wide"><h2>Probe assignments</h2>{sources.length ? sources.map(source => {const current = cook.assignments?.find(item => item.coreDeviceId === source.deviceId && item.probeChannel === source.probe && !item.endedAt); return <form className="assignment" key={`${source.deviceId}:${source.probe}`} onSubmit={async event => {event.preventDefault(); const form = new FormData(event.currentTarget); const measurementId = form.get('measurement'); if (measurementId) await mutate(`/api/v1/cooks/${cook.id}/assignments`, {measurementId, coreDeviceId: source.deviceId, probeChannel: source.probe}); else if (current) await mutate(`/api/v1/assignments/${current.id}`, undefined, 'DELETE');}}><span><strong>{source.deviceId} · Probe {source.probe}</strong><small>{source.available ? `${source.temperatureC}°C` : 'Unavailable'}</small></span><select name="measurement" defaultValue={current?.measurementId ?? ''} disabled={disabled}><option value="">Not used</option>{cook.measurements.map(item => <option value={item.id} key={item.id}>{item.label}</option>)}</select><button disabled={disabled}>Save</button></form>;}) : <p>No thermometer sources are currently available. You can start without probes.</p>}</section>
